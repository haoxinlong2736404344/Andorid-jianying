...
6. 提供 Android Demo，一端可展示合法配置、错误配置和样例切换。
...
如果本机已有 Android SDK、Gradle，并且能访问 Maven Central / Google Maven：
...
当前目录没有 Gradle Wrapper。如本机已安装 Gradle：
...
一句话版本：我把运营高频变化的页面结构、文案、样式、状态、列表和事件从端代码中抽出来，用受控 DSL 表达，再通过 Parser 校验成类型安全 IR，最后由 Compose Renderer 统一渲染，从而让页面能跟随配置和数据变化，同时保留错误提示、工程边界和可测试性。
